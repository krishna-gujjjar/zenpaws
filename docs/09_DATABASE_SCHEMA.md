...
SQLite, WAL mode, FTS5, prepared statements only (no string-concatenated SQL, ever - this is also a security requirement, see `12_SECURITY_MODEL.md`). Accessed exclusively through `zenpaws-database`'s typed query functions; never raw SQL from the frontend (see the `rusqlite` decision in `05_TECHNICAL_DECISIONS.md`).

## Migrations

Managed as sequential, never-edited files (`migrations/0001_init.sql`, `0002_...`). **Never edit an existing migration** - always add a new one, even to fix a mistake in an earlier one. This is what keeps every peer's schema reconstructable from history.
...
- `messages(lamport_peer, lamport_counter)` - sync/conflict resolution lookups (`27_SYNC_REPLICATION_MODEL.md`).
- FTS5's own index covers search; no separate search index needed.

## Why these choices satisfy the 100k-message requirement

Pagination queries against `messages(room, created_at)` with `LIMIT`/keyset pagination (not `OFFSET`, which degrades at scale) plus the virtualized list on the frontend (`@tanstack/react-virtual`) are what make 100,000+ messages scroll smoothly - this is a combined DB + UI concern, cross-reference `13_PERFORMANCE_GUIDELINES.md`.