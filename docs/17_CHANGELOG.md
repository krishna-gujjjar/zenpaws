...
## Oxc root configuration reconciliation

- Applied the manually supplied root Oxfmt/Oxlint/package configuration.
- Removed the conflicting duplicate `.oxlintrc.json`.
- Removed stale Biome editor actions from Zed settings.
- Ran `bun run fix` and manually resolved the remaining Oxlint diagnostics.
- Confirmed `bun run check` and `bun run typecheck` pass.

- Fixed a message-feed runtime crash caused by null or malformed infinite-query page data.

## Network diagnostics

- Added a separate full-screen LAN diagnostics screen.
- Added local address, TCP/UDP socket, mDNS, connected-peer, and recent backend log reporting.
